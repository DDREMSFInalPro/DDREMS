...
const STATUS_CONFIG = {
  ...
  forwarded_to_owner: { label: 'Pending Your Review', badge: 'warning', icon: '⏳' },
  counter_offer: { label: 'Counter-Offer Sent', badge: 'purple', icon: '💰' },
  counter_offer_sent: { label: 'Counter-Offer Delivered', badge: 'purple', icon: '📨' },
...
const AgreementPage = () => {
  ...
  const handleRespond = async (id, action) => {
    ...
    if (!window.confirm(`Are you sure you want to ${verb} this agreement request?`)) return;
  ...
  const pendingRequests = agreements.filter(a => a.status === 'forwarded_to_owner');
  const filteredAgreements = filter === 'all'
    ? agreements.filter(a => a.status !== 'forwarded_to_owner')
    : agreements.filter(a => a.status === filter && a.status !== 'forwarded_to_owner');

  const openCounterOffer = (a) => {
    setCounterOfferModal(a.id);
    setCounterPrice(a.agreementType === 'rental' ? (a.monthlyRent || '') : (a.salePrice || a.property?.price || ''));
    setCounterNotes('');
  };
  ...
  return (
    <div className="agreement-page">
      <div className="page-header">
        <div>
          <h1>Agreements</h1>
          <p className="page-header__subtitle">Review buyer requests and manage agreements</p>
        </div>
      </div>
      ...
      <div className="workflow-banner">
        ...
        <div className="workflow-banner__content">
          <strong>How it works:</strong> When a buyer requests an agreement, the property administrator forwards the request to you for review.
          Accept or reject the request below. Once approved, the administrator will generate the official agreement document.
        </div>
      </div>
      ...
      <div className="agreement-section">
        <div className="card">
          <div className="card__header">
            <h3>⏳ Incoming Requests (Forwarded by Admin) {pendingRequests.length > 0 && <span className="badge badge--warning" style={{marginLeft: '0.5rem'}}>{pendingRequests.length}</span>}</h3>
          </div>
                  ...
                  <div key={a.id} className="agreement-request-card">
                    <div className="agreement-request-card__header">
                      <div className="agreement-request-card__property">
                        ...
                        <span className={`badge badge--${a.agreementType === 'sale' ? 'purple' : 'info'}`}>{a.agreementType}</span>
                        <span className="reference-code">ID: {a.id.slice(0, 8).toUpperCase()}</span>
                      </div>
                    ...
                    <div className="agreement-request-card__body">
                      <div className="agreement-request-card__detail-grid">
                        <div className="agreement-request-card__detail">
                          <span className="agreement-request-card__label">Buyer</span>
                          <span className="agreement-request-card__value">{a.buyer?.name || a.clientName}</span>
                        </div>
                        <div className="agreement-request-card__detail">
                          <span className="agreement-request-card__label">Email</span>
                          <span className="agreement-request-card__value">{a.buyer?.email || a.clientEmail || '—'}</span>
                        </div>
                        <div className="agreement-request-card__detail">
                          <span className="agreement-request-card__label">Phone</span>
                          <span className="agreement-request-card__value">{a.buyer?.phone || a.clientPhone || '—'}</span>
                        </div>
                        <div className="agreement-request-card__detail">
                          <span className="agreement-request-card__label">Property Address</span>
                          <span className="agreement-request-card__value">{a.property?.address || '—'}</span>
                        </div>
                        {a.agreementType === 'rental' && a.monthlyRent && (
                          <div className="agreement-request-card__detail">
                            <span className="agreement-request-card__label">Monthly Rent</span>
                            <span className="agreement-request-card__value">ETB {Number(a.monthlyRent).toLocaleString()}</span>
                          </div>
                        )}
                        {a.agreementType === 'sale' && a.salePrice && (
                          <div className="agreement-request-card__detail">
                            <span className="agreement-request-card__label">Sale Price</span>
                            <span className="agreement-request-card__value">ETB {Number(a.salePrice).toLocaleString()}</span>
                          </div>
                        )}
                      </div>
                      {a.terms && (
                        <div className="agreement-request-card__terms">
                          <span className="agreement-request-card__label">Terms</span>
                          <p>{a.terms}</p>
                        </div>
                      )}
                      {a.adminNotes && (
        ...
        <div className="card">
          <div className="card__header">
            ...
            <div className="agreement-filters">
              {['all', 'pending', 'owner_approved', 'owner_rejected', 'completed'].map((f) => (
                <button
                  key={f}
          ...
          <div className="card__body">
            {filteredAgreements.length === 0 ? (
              <div className="empty-state"><p>No agreements found.</p></div>
            ) : (
                  ...
                  return (
                    <div key={a.id} className="agreement-item">
                      <div className="agreement-item__info">
                        ...
                        <span className={`badge badge--${cfg.badge}`}>{cfg.icon} {cfg.label}</span>
                      </div>
                      ...
                      {a.pdfUrl && a.status === 'completed' && (
                        <a href={a.pdfUrl} target="_blank" rel="noopener noreferrer" className="btn btn--sm btn--outline">📥 Download</a>
                      )}
                    </div>
                  );
                })}
      ...
      {/* Counter-Offer Modal */}
      {counterOfferModal && (
        <div className="modal-overlay" onClick={() => setCounterOfferModal(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal__header">
              <h2>💰 Make Counter-Offer</h2>
              <button className="modal__close" onClick={() => setCounterOfferModal(null)}>✕</button>
            </div>
            <div className="modal__body">
              ...
              <div className="modal__footer">
                <button className="btn btn--outline" onClick={() => setCounterOfferModal(null)}>Cancel</button>
                <button className="btn btn--primary" onClick={handleCounterOffer} disabled={actionLoading}>
