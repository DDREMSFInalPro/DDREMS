...
const { Agreement, Property, User, PropertyImage } = require('../models');
...
const getAgreements = async (req, res, next) => {
  try {
    ...
    const { count, rows } = await Agreement.findAndCountAll({
      where,
      include: [
        { model: Property, as: 'property', attributes: ['id', 'title', 'address', 'price', 'listingType'] },
      ...
      include: [
        {
          ...
          include: [{ model: PropertyImage, as: 'images', attributes: ['id', 'imageUrl', 'isPrimary'] }],
        },
        { model: User, as: 'buyer', attributes: ['id', 'name', 'email', 'phone', 'address'] },
        { model: User, as: 'owner', attributes: ['id', 'name', 'email', 'phone', 'address'] },
      ],
    });
...
const addAdminNote = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, message: 'Admin note updated.', data: agreement });
  } catch (error) {
    next(error);
  }
};
...
const generatePDF = async (req, res, next) => {
  try {
    ...
    if (agreement.status !== 'owner_approved') {
      return res.status(400).json({ success: false, message: 'PDF can only be generated for owner-approved agreements.' });
    }
    ...
    doc.fontSize(14).text('Dire Dawa Real Estate Management System', { align: 'center' });
    doc.moveDown();
    doc.fontSize(16).text(`${agreement.agreementType === 'sale' ? 'Sale' : 'Rental'} Agreement`, { align: 'center' });
    doc.moveDown();
    ...
    doc.text(`Address: ${agreement.property.address}, ${agreement.property.city || 'Dire Dawa'}`);
    doc.text(`Type: ${agreement.property.propertyType} (${agreement.property.listingType})`);
    ...
    if (agreement.agreementType === 'rental') {
      if (agreement.monthlyRent) doc.text(`Monthly Rent: ETB ${Number(agreement.monthlyRent).toLocaleString()}`);
      if (agreement.startDate) doc.text(`Start Date: ${agreement.startDate}`);
      if (agreement.endDate) doc.text(`End Date: ${agreement.endDate}`);
    } else {
      if (agreement.salePrice) doc.text(`Sale Price: ETB ${Number(agreement.salePrice).toLocaleString()}`);
    }
    if (agreement.terms) {
      doc.moveDown(0.5);
    ...
    res.json({
      ...
      data: { id: agreement.id, pdfUrl: agreement.pdfUrl, status: agreement.status },
    });
  } catch (error) {
    next(error);
  }
...
const forwardToOwner = async (req, res, next) => {
  try {
    ...
    if (agreement.status !== 'pending') {
      return res.status(400).json({ success: false, message: 'Only pending agreements can be forwarded to the owner.' });
    }
...
const sendCounterOfferToBuyer = async (req, res, next) => {
  try {
    ...
    if (agreement.status !== 'counter_offer') {
      return res.status(400).json({ success: false, message: 'Only counter-offer agreements can be sent to the buyer.' });
    }
...
module.exports = { getAgreements, getAgreementById, addAdminNote, generatePDF, forwardToOwner, sendCounterOfferToBuyer };
