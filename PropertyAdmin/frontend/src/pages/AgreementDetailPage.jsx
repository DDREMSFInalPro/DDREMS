...
const AgreementDetailPage = () => {
  ...
  const fetchAgreement = async () => {
    ...
    try {
      ...
      if (['payment_submitted','payment_confirmed','payment_verified','completed'].includes(res.data.data.status)) {
        try {
  ...
  const handleVerifyPayment = async () => {
    if (!window.confirm("Verify this payment and allow PDF generation?")) return;
    setActionLoading(true);
    try {
      ...
      if (res.data.data?.receiptPdfUrl) {
        setPayment((p) => ({ ...p, receiptPdfUrl: res.data.data.receiptPdfUrl }));
      }
      fetchAgreement();
    } catch (err) {
  ...
  const handleAskOwnerPayment = async () => {    if (!window.confirm("Notify the owner to confirm payment receipt?")) return;
    setActionLoading(true);
    try {
  ...
  const handleGeneratePDF = async () => {
    if (!window.confirm("Generate PDF for this agreement?")) return;    setActionLoading(true);
    try {
    ...
    <div className="agreement-detail-page">
      ...
      {payment && (
        <div className="card" style={{ marginTop: "1rem" }}>
          <div className="card__header"><h3>💳 Payment Submitted by Buyer</h3></div>
          <div className="card__body">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem 2rem" }}>
              <p><strong>Gateway:</strong> {payment.paymentGateway}</p>
              ...
              <p><strong>Owner Confirmed:</strong>{" "}
                {payment.ownerConfirmed
                  ? <span style={{ color: "green" }}>✅ Yes — {new Date(payment.ownerConfirmedAt).toLocaleString()}</span>
                  : <span style={{ color: "#f59e0b" }}>⏳ Pending</span>}
              </p>
            </div>
            {payment.receiptUrl && (
              <div style={{ marginTop: "1rem" }}>
                <p><strong>📎 Buyer Receipt:</strong></p>
                {payment.receiptUrl.match(/\.(jpg|jpeg|png|webp)$/i) ? (
                  ...
                  <a href={payment.receiptUrl} target="_blank" rel="noopener noreferrer" className="btn btn--sm btn--outline" style={{ marginTop: "0.5rem" }}>📎 Open Receipt</a>
                )}
              </div>
            )}
            {!payment.receiptUrl && (
              <p style={{ marginTop: "0.75rem", color: "#6b7280" }}>No receipt image uploaded by buyer.</p>
            )}
            {payment.receiptPdfUrl && (
              <div style={{ marginTop: "0.75rem" }}>
                <a href={payment.receiptPdfUrl} target="_blank" rel="noopener noreferrer" className="btn btn--sm btn--primary">📄 Download Verified Receipt PDF</a>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="card__body">
          <div className="action-buttons">
            ...
            {agreement.status === "payment_submitted" && (
              <>
                <div className="alert alert--info" style={{ marginBottom: "1rem" }}>
                  Buyer has submitted payment. Ask the owner to confirm receipt before generating the PDF.
                </div>
                <button className="btn btn--warning" onClick={handleAskOwnerPayment} disabled={actionLoading}>
                  {actionLoading ? "⏳ Processing..." : "📨 Ask Owner to Confirm Payment"}
                </button>
              </>
            )}
            {agreement.status === "payment_confirmed" && (
              <>
                <div className="alert alert--info" style={{ marginBottom: "1rem" }}>
                  ✅ Owner confirmed payment received. Review the payment details above and verify to proceed.
                </div>
                <button className="btn btn--success" onClick={handleVerifyPayment} disabled={actionLoading}>
            ...
            {agreement.status === "payment_verified" && !agreement.pdfUrl && (
              <>
                {payment?.receiptPdfUrl && (
                  <div className="alert alert--info" style={{ marginBottom: "1rem" }}>
                    <a href={payment.receiptPdfUrl} target="_blank" rel="noopener noreferrer" className="btn btn--sm btn--outline">📄 Download Payment Receipt PDF</a>
                  </div>
                )}
                <button className="btn btn--primary" onClick={handleGeneratePDF} disabled={actionLoading}>
                  {actionLoading ? "⏳ Processing..." : "📄 Generate Agreement PDF & Complete"}
                </button>
              </>
            )}
